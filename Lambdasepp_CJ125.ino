...
#define           UBAT_MIN                            450           /* Minimum voltage (ADC value) on Ubat to operate - Value depending on used voltage divider for 10.8V Ub */
...
if ( adcValue_UB < UBAT_MIN ) {
       ...
       mySerial.print("UBAT low.\n\r");
}

   else if ( n >= 9 & CJ125_Status != CJ125_DIAG_REG_STATUS_OK ) {
     // If CJ125 can not be found than put out a error message
     mySerial.print("CJ125 not ready.\n\r");
     mySerial.print("CJ125 Status: ");
     mySerial.print(CJ125_Status);
     mySerial.print(" \n\r");
     
     // Stop programm and do nothing 
     while ( n >=9 ) {} 
   }
...
void loop() {
  ...
  if ( (100 / SERIAL_RATE) ==  serial_counter) {
    ...
    if (CJ125_Status == CJ125_DIAG_REG_STATUS_OK) {
      ...
      // Lambda DAC Output 
      DAC_Counter = LAMBDA_VALUE * 4;
      dac.setVoltage(DAC_Counter, false);
