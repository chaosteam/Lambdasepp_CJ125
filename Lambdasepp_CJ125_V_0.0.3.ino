...
//   ||                  11 = No Failure
//   ||---------- Ext. Heater DIAHD Diagnostic Bit 0
//   |----------- Ext. Heater DIAHG Diagnostic Bit 1
//                            00 = Short circuit to ground
//                            01 = Open Load
//                            10 = Short circuit to Vbatt
//                            11 = No Failure
...
#define           LAMBDA_PWM_OUTPUT_PIN               11            /* Analog output to ECU.*/
...
void loop() {
  ...
  if ( (100 / SERIAL_RATE) ==  serial_counter) {
    ...
    if (CJ125_Status == CJ125_DIAG_REG_STATUS_OK) {
      ...
      //Analog PWM Output
      float PWM_OUT = (AFR_VALUE - 10) * 25.5;
      if (AFR_VALUE < 10) float PWM_OUT = 0;
      if (AFR_VALUE > 20) float PWM_OUT = 255;
      analogWrite(LAMBDA_PWM_OUTPUT_PIN,255);
